...
# Define Monocle3-like palette
purple_yellow_palette <- c("#352A86", "#4B2991", "#633C9F", "#7C519D", "#995D95", "#B4688C", "#CE7B81", "#E48F75", "#F7A761", "#FDB32A", "#F7D622", "yellow")
...
simplex2_with_pseudotime <- function(points, pseudotime, max_pseudotime, origin = c(0, 0), show_points = TRUE, 
  ...
  # Create a ggplot with pseudotime as color using the purple-to-yellow palette
  p <- ggplot(points_projected, aes(x = X1, y = X2, color = pseudotime)) +
    geom_point(size = point_size, alpha = 0.6) +
    scale_color_gradientn(colors = purple_yellow_palette, limits=c(0, max_pseudotime)) +  # Apply purple-to-yellow color scale
    theme_void() +
...
```{r}
df_subset <- df[rownames(df) %in% rownames(data_pseudotime), ]
print(nrow(df_subset ))
head(df_subset)
```

```{r}
df_combined <- merge(df_subset, data_pseudotime, by = "row.names", all.y = TRUE)
rownames(df_combined) <- df_combined$Row.names
df_combined$Row.names <- NULL
df_combined <- df_combined[match(rownames(data_pseudotime), rownames(df_combined)), ]
head(df_combined)
...
which_rows <-  df_combined$seurat_clusters== "Chondrocytes" | df_combined$seurat_clusters== "Osteoblasts" | df_combined$seurat_clusters == "1-3 dpjr Mesenchyme" |  df_combined$seurat_clusters == 'Chondrogenic Fibroblasts'
subset_to_plot <- df_combined[  which_rows, ]
